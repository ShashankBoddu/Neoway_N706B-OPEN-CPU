...
bool nwy_hal_os_thread_create(nwy_osi_thread_t *thread_out, const char *name,
  ...
  // Convert generic thread function type to SDK thread entry pointer
  // (nwy_osi_cb_func)
  nwy_error_e ret =
      nwy_thread_create(thread_out, (char *)name, (uint8)priority,
                        (nwy_osi_cb_func)func, param, 10, stack_size, NULL);
  return (ret == NWY_SUCCESS);
}

void nwy_hal_os_thread_sleep(uint32_t ms) { nwy_thread_sleep(ms); }

bool nwy_hal_os_timer_create(nwy_osi_timer_t *timer_out,
                             nwy_hal_os_timer_cb_t cb, void *cb_para) {
  if (!timer_out || !cb)
    return false;
...
bool nwy_hal_os_timer_start(nwy_osi_timer_t timer, uint32_t interval_ms,
                            nwy_hal_os_timer_type_e type) {
  if (!timer)
    return false;
  ...
  timer_para.type = (type == NWY_HAL_OS_TIMER_PERIODIC) ? NWY_TIMER_PERIODIC
                                                        : NWY_TIMER_ONE_TIME;
...
bool nwy_hal_os_semaphore_create(nwy_osi_semaphore_t *sem_out,
...
bool nwy_hal_os_semaphore_acquire(nwy_osi_semaphore_t sem,
                                  uint32_t timeout_ms) {
  if (!sem)
    return false;
