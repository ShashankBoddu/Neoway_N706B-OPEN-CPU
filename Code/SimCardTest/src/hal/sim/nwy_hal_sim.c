...
bool nwy_hal_sim_get_msisdn(int sim_id, char *msisdn_out, int max_len) {
    if (!msisdn_out || max_len <= 0) return false;
...
bool nwy_hal_sim_change_pin(int sim_id, const char *old_pin, const char *new_pin) {
    if (!old_pin || !new_pin) return false;
    return (nwy_sim_pin_change(get_nwy_sim_id(sim_id), (char *)old_pin, (char *)new_pin) == 0);
}

bool nwy_hal_sim_verify_puk(int sim_id, const char *puk, const char *new_pin) {
    if (!puk || !new_pin) return false;
    return (nwy_sim_pin_unlock(get_nwy_sim_id(sim_id), (char *)puk, (char *)new_pin) == 0);
}

bool nwy_hal_sim_get_retry_times(int sim_id, uint8_t *pin_attempts, uint8_t *puk_attempts) {
    if (!pin_attempts || !puk_attempts) return false;
    return (nwy_sim_pin_times_get(get_nwy_sim_id(sim_id), pin_attempts, puk_attempts) == 0);
}
...
bool nwy_hal_sim_csim_command(int sim_id, const char *command, uint8_t length, char *response_out, int response_max_len) {
    if (!command || !response_out || response_max_len <= 0) return false;
    return (nwy_sim_csim(get_nwy_sim_id(sim_id), (char *)command, length, response_out, response_max_len) == 0);
}

static void internal_sim_cb(nwy_sim_id_e sim_id, nwy_sim_urc_type_e urc_type, nwy_sim_info_ind_t *ind_struct) {
    int user_sim_id = (sim_id == NWY_SIM_ID_SLOT_2) ? 2 : 1;
    int sim_idx = user_sim_id - 1;
    
    if (sim_idx < 0 || sim_idx >= 2) return;
...
bool nwy_hal_sim_register_urc_cb(int sim_id, nwy_hal_sim_urc_callback_t cb) {
    ...
    if (sim_idx < 0 || sim_idx >= 2) return false;
