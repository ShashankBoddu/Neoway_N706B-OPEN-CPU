...
static void handle_internal_data_cb(int sim_id, int profile_idx,
...
static void internal_data_cb_sim1(int profile_idx,
                                  nwy_data_call_state_e ind_state) {
  handle_internal_data_cb(1, profile_idx, ind_state);
}

static void internal_data_cb_sim2(int profile_idx,
                                  nwy_data_call_state_e ind_state) {
...
bool nwy_hal_net_is_registered(int sim_id) {
  ...
  if (nwy_nw_regstatus_get(nwy_hal_get_sim_id_enum(sim_id), &reg_status) == 0) {
    if ((reg_status.cs_regs_valid &&
         reg_status.cs_regs.regs_state == NWY_NW_SERVICE_FULL) ||
        (reg_status.ps_regs_valid &&
         reg_status.ps_regs.regs_state == NWY_NW_SERVICE_FULL)) {
      return true;
    }
  }
  return false;
...
bool nwy_hal_net_get_operator_name(int sim_id, char *name_out, int max_len) {
  ...
  if (nwy_nw_operator_get(nwy_hal_get_sim_id_enum(sim_id), &opt) == 0) {
    if (strlen(opt.long_eons) > 0) {
      ...
      snprintf(name_out, max_len, "%s (%s%s)", opt.short_eons, opt.mcc,
               opt.mnc);
    } else {
...
bool nwy_hal_net_get_registration_details(int sim_id, int *cs_state,
                                          int *ps_state, int *rat) {
  nwy_nw_regstatus_t reg_status;
  memset(&reg_status, 0, sizeof(reg_status));

  if (nwy_nw_regstatus_get(nwy_hal_get_sim_id_enum(sim_id), &reg_status) == 0) {
    if (cs_state)
      *cs_state = reg_status.cs_regs_valid ? reg_status.cs_regs.regs_state
                                           : NWY_NW_SERVICE_NONE;
    if (ps_state)
      *ps_state = reg_status.ps_regs_valid ? reg_status.ps_regs.regs_state
                                           : NWY_NW_SERVICE_NONE;
    if (rat) {
      if (reg_status.ps_regs_valid &&
          (reg_status.ps_regs.regs_state == NWY_NW_SERVICE_FULL ||
           reg_status.ps_regs.regs_state == NWY_NW_SERVICE_LIMITED)) {
        *rat = reg_status.ps_regs.rat_type;
      } else if (reg_status.cs_regs_valid &&
                 (reg_status.cs_regs.regs_state == NWY_NW_SERVICE_FULL ||
                  reg_status.cs_regs.regs_state == NWY_NW_SERVICE_LIMITED)) {
...
bool nwy_hal_net_get_operator_details(int sim_id,
...
bool nwy_hal_net_get_custom_cfg(int sim_id, nwy_nw_config_type_e cfg_option,
                                nwy_nw_config_info_u *cfg_info) {
  if (!cfg_info)
    return false;
  return (nwy_nw_config_get(nwy_hal_get_sim_id_enum(sim_id), cfg_option,
                            cfg_info) == 0);
}

bool nwy_hal_net_set_custom_cfg(int sim_id, nwy_nw_config_type_e cfg_option,
                                nwy_nw_config_info_u *cfg_info) {
  if (!cfg_info)
    return false;
  return (nwy_nw_config_set(nwy_hal_get_sim_id_enum(sim_id), cfg_option,
                            cfg_info) == 0);
}
...
bool nwy_hal_net_get_cellinfo(int sim_id, nwy_nw_cellinfo_mode_e scan_mode,
                              nwy_nw_cellinfo_t *info) {
  if (!info)
    return false;
  return (nwy_nw_cellinfo_get(nwy_hal_get_sim_id_enum(sim_id), scan_mode,
                              info) == 0);
}

bool nwy_hal_net_start_data_call(int sim_id, int profile_idx,
                                 nwy_hal_net_callback_t cb) {
  int sim_idx = sim_id - 1;
  if (sim_idx < 0 || sim_idx >= 2 || profile_idx < 1 || profile_idx > 6) {
    return false;
  }
  ...
  nwy_data_reg_cb(nwy_hal_get_sim_id_enum(sim_id), profile_idx,
                  (sim_id == 2) ? internal_data_cb_sim2
                                : internal_data_cb_sim1);
  ...
  return (nwy_data_call_start(nwy_hal_get_sim_id_enum(sim_id), &call_param) ==
          0);
}
...
bool nwy_hal_net_get_ip(int sim_id, int profile_idx, char *ip_out,
                        int max_len) {
  if (!ip_out || max_len <= 0 || profile_idx < 1 || profile_idx > 6)
    return false;
  nwy_data_callinfo_t call_info;
  memset(&call_info, 0, sizeof(call_info));
  if (nwy_data_call_info_get(profile_idx, &call_info) == 0) {
    if (strlen(call_info.ipv4_str) > 0 &&
        strcmp(call_info.ipv4_str, "0.0.0.0") != 0) {
