...
void nwy_hal_uart_set_log_fd(int fd) { g_log_fd = fd; }

static void uart_log_format(char level_char, const char *tag, const char *fmt,
                            va_list args) {
  if (g_log_fd < 0 || !fmt || !tag)
    return;
