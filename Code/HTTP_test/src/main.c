...
// Directly handle SDK HTTP response
static void http_event_callback(nwy_http_result_t *result) {
  if (!result)
    return;
  ...
  case NWY_HTTPS_SSL_CONNECTED:
    LOGI("HTTP Channel Opened Successfully. Sending GET...");
    http_get_param_t get_param = {0};
    get_param.uri = "/";
    get_param.keepalive = 0;
    nwy_http_get(result->http_handle, &get_param);
    break;
  case NWY_HTTP_DATA_RECVED:
    LOGI("HTTP Response (%.*s)", result->data_len, result->data);
    break;
  case NWY_HTTP_CLOSED:
  case NWY_HTTP_CLOSED_PASV:
    LOGI("HTTP Channel Closed by Server.");
    break;
  case NWY_HTTP_DNS_ERR:
  case NWY_HTTP_OPEN_FAIL:
  case NWY_HTTP_EVENT_SSL_CONNECT_FAIL:
    LOGE("HTTP Channel Error Event: %d", result->event);
    break;
  default:
    LOGI("HTTP Event ID: %d", result->event);
    break;
  }
}

// When triggering the test:
static void execute_automated_http_flow(void) {
  LOGI("STAGE 5 HTTP: Initializing HTTP Setup...");
  ...
  nwy_http_handle_t h = nwy_http_setup(&param, NULL);
  if (h == NULL) {
